...
#   - The RedNote server speaks HTTP, so it has to be running. It is started
...
RN_BIN="$REPO/servers/xiaohongshu/bin/xiaohongshu-mcp"
if [[ ! -x "$RN_BIN" ]]; then
  log "rednote binary missing — run: make build"
  exit 1
fi

RN_HEALTH="http://localhost:18060/health"
...
if curl -sf "$RN_HEALTH" >/dev/null 2>&1; then
  log "rednote server already up — adopting it, leaving it running"
else
  ...
  sleep 1
  ( cd "$REPO/servers/xiaohongshu" && "$RN_BIN" -headless=true ) >"$LOG_DIR/rednote.log" 2>&1 &
  RN_PID=$!
  trap 'kill $RN_PID 2>/dev/null; wait $RN_PID 2>/dev/null' EXIT

  for _ in $(seq 20); do
    curl -sf "$RN_HEALTH" >/dev/null 2>&1 && break
    sleep 1
  done
  if ! curl -sf "$RN_HEALTH" >/dev/null 2>&1; then
    log "rednote server did not come up — running without it"
  fi
fi
